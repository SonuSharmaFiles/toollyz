...
export const tools: Tool[] = [
  ...
  },
  {
    ...
    seo: {
      ...
      relatedSlugs: ["fancy-text-generator", "case-converter", "word-counter", "secure-notes"],
      faqs: [
        { q: "Does this call ChatGPT or Claude to improve my prompt?", a: "No — the whole tool is heuristic, client-side. We pattern-match for the 8 prompt components and compose a structured rewrite. That keeps your prompt private and the tool free." },
        { q: "What are the 8 components scored?", a: "Role (you are a …), task (action verb), context (background), constraints (rules and limits), format (output shape), examples (few-shot pairs), audience (who it's for), tone (voice/style). They're weighted in the 100-point score." },
        { q: "Why is the score sometimes low even though the prompt is short and clear?", a: "Short prompts often miss audience, tone, format and examples — even if the task is clear. The 8-check grid shows which ones are missing so you can decide whether to add them." },
        { q: "Which rewrite style should I use?", a: "Markdown sections are the universal default and work everywhere. XML tags are documented to give the best results with Claude. ALL-CAPS sections are a plain-text fallback for older or stricter models." },
        { q: "What if the rewrite has placeholders I don't want?", a: "Edit them out — the rewrite is a scaffold, not a final prompt. Removing a section you don't need is faster than typing it from scratch." },
        { q: "Does it work in other languages?", a: "The pattern matching is English-centric, so the score will be lower for non-English prompts. The structural scaffold still works for any language — just translate the section headings." },
        { q: "How big can the prompt be?", a: "There's no hard cap. Larger prompts work but the analysis runs on every keystroke (debounced) — multi-thousand-word prompts may feel slightly less responsive." },
        { q: "Is my prompt logged or stored?", a: "Only locally — your last prompt and style choice are saved to localStorage on this device. Toollyz has no server that ever sees the text." },
        { q: "Can I share the enhanced prompt?", a: "Yes — copy or download the rewrite as Markdown, XML or .txt and paste into any LLM. Markdown reads cleanly in Cursor, ChatGPT, Claude and most other UIs." },
        { q: "Should I always use the rewrite verbatim?", a: "No — treat it as a scaffold. Fill in the placeholders with real context, remove sections you don't need and adjust the tone. The structure is the value, the words inside are yours." },
        { q: "Why isn't there an example library?", a: "Examples are highly task-specific — we surface a placeholder block and trust you to add a real input/output pair. A generic example library would dilute the signal." },
      ],
    },
  },
  ...
  // ─── TEXT (batch 2) ──────────────────────────────────────────────────────
  {
    ...
    seo: {
      ...
      faqs: [
        ...
        { q: "How is mirror text different from upside-down text?", a: "Mirror text replaces each letter with its right-to-left mirror counterpart but keeps the original order, so the result reads correctly held up to a mirror. Upside-down text reverses the order and uses inverted glyphs, so the result reads correctly when rotated 180°." },
        { q: "Will emoji break when I reverse?", a: "No. Toollyz splits the string by Unicode code points (using the spread operator), so multi-byte characters like emoji and astral-plane glyphs are kept whole instead of being split into half-pairs." },
        { q: "Why would I want to reverse only the word order?", a: "Reversing word order keeps each word readable but flips the sentence direction — handy for grammar exercises, Yoda-style writing, or testing right-to-left layout in design tools." },
        { q: "Can I reverse multiple lines at once?", a: "Yes. The 'Line order' mode flips your top line to the bottom; the 'Each line' mode keeps the line order but reverses the characters inside each line; 'Each word' reverses only the characters inside each word." },
        { q: "Is anything uploaded?", a: "No. The entire reversal runs in your browser — Toollyz has no backend that ever sees your text." },
        { q: "How long can the input be?", a: "The reverser handles inputs of millions of characters comfortably; the browser textarea is the practical limit. We use React's deferred values so typing stays smooth even on large strings." },
        { q: "Will reversed Unicode work on Twitter, Instagram and Discord?", a: "Yes — the upside-down and mirror outputs are standard Unicode characters that render in any platform that supports Unicode (which is essentially everywhere)." },
        { q: "Does it preserve newlines and indentation?", a: "Yes. The line-aware modes split on newline characters and process each line, preserving the original line structure. Word reversal preserves whitespace runs so indentation stays meaningful." },
        { q: "How do I reverse text back to normal?", a: "Just run the reverser again on the output with the same mode — the operation is its own inverse for character, word and line reversal. The 'Use output as input' button does that in one click." },
      ],
    },
  },
  {
    ...
    seo: {
      ...
      faqs: [
        ...
        { q: "What is the lexical-diversity score?", a: "It's the ratio of unique words to total words after the same normalisation, expressed as a percentage. Higher is more varied vocabulary — 70%+ is typical for tight prose; lower numbers suggest the writing leans on the same words too often." },
        { q: "Can I increase the minimum word length?", a: "Yes. Bump 'min length' up to 4 or 5 to filter out short tokens like 'I', 'is' and 'it'. The default 3 already removes 1- and 2-letter words." },
        { q: "Will it work on non-English text?", a: "Yes for tokenisation — the regex uses Unicode letter/number classes, so French, Spanish, German, Hindi and so on tokenise correctly. The stopword list is currently English-only; turn off the toggle for other languages." },
        { q: "Why are only the top 5 words colour-highlighted?", a: "Past five distinct colours, the highlighting starts to look like a rainbow and obscures the text. The frequency table below lists every duplicate so nothing is lost." },
        { q: "Can I export the report?", a: "Yes — copy the duplicates as a TSV (word [tab] count) or download a .tsv file for spreadsheet use." },
        { q: "How does this differ from a word counter?", a: "A word counter tells you the total count and unique-word count. The duplicate finder ranks every individual repeated word so you can act on each one." },
        { q: "Does this suggest synonyms?", a: "Not yet — we keep the tool focused and 100% offline. A thesaurus would either need a network dependency or a heavy bundle. Use the highlights to spot repeats and look them up in your favourite reference." },
        { q: "Is my text uploaded anywhere?", a: "No. Tokenisation, counting and highlighting all happen in your browser — Toollyz has no backend that ever sees your writing." },
        { q: "How big can the input be?", a: "Comfortable up to tens of thousands of words. The work is O(n) for tokenising and O(unique) for highlighting, both fast in the browser thanks to deferred values that keep typing smooth." },
      ],
    },
  },
  {
    ...
    seo: {
      ...
      faqs: [
        ...
        { q: "What's wrong with em-dashes?", a: "Nothing in moderation. But ChatGPT-style prose chains em-dashes ('X — Y — Z') reliably, which is a tell. We replace 'X — Y —' patterns with parentheses for variety, but leave single em-dashes alone." },
        { q: "What's the 'tricolon' detection?", a: "The 'X, Y, and Z' three-item list pattern. LLMs lean on it heavily. We don't auto-rewrite tricolons (they're often the clearest phrasing) but we count them so you can decide if your draft has too many." },
        { q: "What does each persona do differently?", a: "Balanced: light contractions, mild edits — safe for formal contexts. Casual: more contractions, occasional sentence fragments, broken rhythm — for blog / social. Expert: strips hedges ('one could argue', 'arguably'), confident voice. Story: replaces abstract phrases ('in recent years' → 'lately') with concrete ones, varied rhythm." },
        { q: "Will it touch my technical vocabulary?", a: "Mostly no — the verb replacements target specific overused verbs (delve, leverage, utilize, elucidate, facilitate). Technical terms pass through." },
        { q: "What's the sentence-length variance score?", a: "A rough proxy for rhythm — standard deviation of sentence word counts. Higher variance reads more human because real writers don't produce 18-word sentences in a row." },
        { q: "Will my text be detectable as AI-generated after humanising?", a: "Maybe still yes by classifiers — but a human reader is more likely to enjoy reading it. That's the goal." },
        { q: "Can I undo specific transformations?", a: "Not in this version — the rewrite is atomic. If a particular swap reads worse, edit the output directly or try a different persona." },
        { q: "Why a 'Story' persona?", a: "Storytelling prose has a different baseline rhythm — concrete nouns, shorter sentences, fewer abstractions. The Story persona biases toward those patterns." },
        { q: "Is anything uploaded?", a: "No. The whole humanisation pass runs in your browser." },
      ],
    },
  },
  {
    ...
    seo: {
      ...
      how: [
        "Pick a mode: Single sentence (8 variations) or Whole text (each sentence rewritten).",
        "Paste your sentence or paragraph.",
        "Choose a tone preset and how many variations you want per sentence (2-8).",
        "Pick the rewrite you like, copy it, or download all the variations as .txt.",
      ],
      benefits: [
      ...
      faqs: [
        ...
        { q: "How does passive ↔ active work?", a: "Passive mode looks for 'Subject verb object' patterns with a small allowlist of verbs (built, made, created, designed, …) and converts to 'Object was verb-ed by subject'. Active mode reverses 'X was Y by Z' → 'Z Y X'. Complex sentences may fall back to the original." },
        { q: "What does 'concise' mode drop?", a: "Filler phrases: 'in order to' → 'to', 'due to the fact that' → 'because', 'at this point in time' → 'now', 'just', 'actually', 'basically', 'very', 'really', 'quite'. The list is curated, not generative." },
        { q: "How many variations do I get?", a: "Two to eight per sentence in single-sentence mode (your choice). Whole-text mode applies the same per-sentence count across the paragraph." },
        { q: "Will the rewrites all be different?", a: "Each one applies the same tone but a different synonym-swap intensity, so you'll see graduated variations. Sometimes two variations may be identical if no synonyms apply — we dedupe and you'll see the actual unique count." },
        { q: "Does it work on non-English text?", a: "The thesaurus is English-only, but the structural transformations (contractions, voice) are tied to English grammar too. For other languages, the output will mostly pass through unchanged." },
        { q: "Will my rewrites be stored?", a: "Only your last input and tone choice persist in localStorage on this device. The rewrites themselves live only in page state." },
        { q: "Why is the AI Text Humanizer separate?", a: "Different job. Sentence Rewriter produces alternatives of the SAME sentence with varying tone. AI Text Humanizer takes AI-style prose and breaks the telltale LLM patterns (stiff transitions, em-dash abuse, tricolons). Use rewriter for drafting, humanizer for cleaning up AI output." },
        { q: "Will it help with academic writing?", a: "Use the formal tone for tightening. Be careful with the synonym thesaurus — academic vocabulary is field-specific and our generic swaps may not preserve discipline-specific connotations." },
        { q: "Is anything uploaded?", a: "No. Rewriting runs entirely in your browser." },
      ],
    },
  },
  {
    ...
    seo: {
      ...
      faqs: [
        ...
        { q: "How does emoji → text handle compound emoji?", a: "Toollyz uses Intl.Segmenter for grapheme-level splitting, so ZWJ-joined sequences (👨‍🚀, 👩‍❤️‍👨) and skin-tone modifiers are treated as a single emoji. Each known emoji becomes `:keyword:`." },
        { q: "Does it handle plurals like 'cats' → 🐱?", a: "Yes — if a word ends in 's' and the singular form is in the dictionary, the singular emoji is used." },
        { q: "Is there a way to add my own entries?", a: "Not in the UI today. The dictionary is a static module — fork the repo on GitHub if you'd like to contribute additions." },
        { q: "Will the output work on Discord / Slack / WhatsApp?", a: "Yes. The output is standard Unicode emoji code points that render anywhere fonts support them." },
        { q: "What's the 'coverage' percentage?", a: "In text → emoji direction it's the fraction of words that matched a dictionary entry. In the reverse direction it's the fraction of emoji in your input that were recognised." },
        { q: "Why are short words like 'is' or 'a' ignored?", a: "Stopwords and very-short tokens generally don't have a good single-emoji counterpart, so they pass through. This keeps the output readable rather than peppered with random emoji." },
        { q: "Can I translate paragraphs?", a: "Yes. The whole input is processed, preserving original spacing and punctuation so the result reads naturally." },
        { q: "Is my text uploaded anywhere?", a: "No. The dictionary is bundled with the page; translation is a pure browser computation." },
        { q: "Why does the same word always pick the same emoji?", a: "The dictionary is keyword-keyed — each word maps to one canonical emoji. Multi-emoji synonyms (love → ❤️ vs 🥰) are picked by the first dictionary entry that lists the keyword." },
      ],
    },
  },
  {
  ...
  },
  {
    ...
    seo: {
      ...
      how: [
        "Paste a YouTube URL or 11-character video ID into the field.",
        "Type your timestamp in any common format (1:23, 1h2m3s, 85s, …).",
        "Pick the link style — short, full, embed, or markdown.",
        "For chapters: add rows, label each segment, and copy YouTube's auto-chapter description.",
      ],
      benefits: [
      ...
      faqs: [
        ...
        { q: "What timestamp formats are supported?", a: "Colon-style — 1:23 (m:s) and 1:23:45 (h:m:s) — and compound style — 85s, 2m30s, 1h2m3s. Mixed formats are detected automatically." },
        { q: "How do YouTube auto-chapters work?", a: "YouTube reads the video description for lines starting with timestamps. If at least 3 timestamps appear (the first being 0:00) and each chapter is at least 10 seconds long, the player shows a chapter list. Toollyz validates these rules live." },
        { q: "Will the link preserve a playlist?", a: "Yes. If your input URL has a `list=` parameter, the generated link includes it (toggleable in defaults)." },
        { q: "Why is the markdown style useful?", a: "It's ready to paste into a blog post or README — the rendered output is a clickable timestamp like '1:23' that opens the video at that moment." },
        { q: "Can I add unlimited chapters?", a: "Yes. Add as many rows as you like — the validator warns you if any chapter is shorter than YouTube's 10-second minimum." },
        { q: "Does the embed URL respect autoplay?", a: "It uses the standard `start=` parameter; combine it with `&autoplay=1` in your `<iframe>` if you want autoplay." },
        { q: "What about YouTube Shorts?", a: "Shorts URLs are parsed correctly and the deep link still works — it just opens at the chosen second inside the Shorts player." },
        { q: "Is anything uploaded?", a: "No. URL parsing and link composing are pure browser functions — Toollyz has no server that ever sees your URL." },
        { q: "Can I reorder chapters?", a: "Yes — each row has up/down arrows. Reordering updates the description preview live." },
      ],
    },
  },
  {
    ...
    seo: {
      ...
      how: [
        "Pick a body style — bold, italic, monospace, small-caps and so on.",
        "Toggle 'preserve blank lines' so platforms don't collapse your formatting.",
        "Type your hashtags as a comma-separated list — they get cleaned and prefixed automatically.",
        "Copy the formatted preview and paste into X, Mastodon, Bluesky, Facebook or any other platform.",
      ],
      benefits: [
      ...
      relatedSlugs: ["fancy-text-generator", "linkedin-post-formatter", "instagram-caption-formatter", "tweet-character-counter"],
      faqs: [
        { q: "How can bold text 'survive copy-paste' if platforms strip formatting?", a: "We don't use rich-text markup — we substitute each letter for the Mathematical-Alphanumeric Unicode equivalent (e.g. 'A' → '𝐀'). Those are actual Unicode characters, so they paste unchanged into any platform that supports Unicode (which is essentially everywhere)." },
        { q: "Does this work on X / Twitter?", a: "Yes. Bold, italic, monospace and small-caps all render correctly. The blank-line booster also keeps your line breaks intact." },
        { q: "Does the blank-line trick break accessibility?", a: "Screen readers ignore zero-width / invisible characters, so the audio output is unchanged. Visual rendering gets the gap. It's the standard fix used by every social-media scheduling tool." },
        { q: "Will hashtags survive my style choice?", a: "Yes. We only style the body — the hashtag block at the bottom is left as plain text so platforms recognise it and make tags clickable." },
        { q: "Why do some Android keyboards render strike-through poorly?", a: "Combining-character styles like strikethrough require the rendering engine to draw the combining mark over the previous glyph. Some Android keyboards / older Android versions don't draw them at the right offset. The tool warns when you pick one." },
        { q: "Can I bold only part of a post?", a: "Not in this version — the style applies to the whole body. For mixed styling, copy the bold portion separately, then paste the plain part. Or use the LinkedIn formatter, which has separate hook + body style controls." },
        { q: "Is this the same as fancy-text generators?", a: "It shares the same Unicode transforms as our Fancy Text Generator. The difference is the hashtag block, line-break booster and live character counts geared specifically at posting — not just generating one fancy phrase." },
        { q: "Does dedupe handle case differences?", a: "Yes — duplicates are matched case-insensitively. The original casing of the first occurrence is kept." },
        { q: "What about emoji?", a: "Emoji pass through unchanged. The Unicode transforms only apply to ASCII letters and digits — emoji aren't in the Mathematical-Alphanumeric blocks." },
        { q: "Is anything uploaded?", a: "No. All formatting and previewing runs entirely in your browser." },
        { q: "How many characters / hashtags can I include?", a: "There's no tool-side limit, but most platforms cap posts (280 / 500 / 5 000 chars). The character counter helps you stay within whatever platform you're posting to." },
      ],
    },
  },
  {
    ...
    seo: {
      ...
      faqs: [
        ...
        { q: "What about the 4 000 and 25 000 limits?", a: "Those are the Premium 'long post' (4 000) and Premium+ (25 000) ceilings. Switch tier in the picker and the counter, ring gauge and thread-split budget all update." },
        { q: "How does thread splitting work?", a: "It greedily packs words into each tweet until adding another would exceed the budget. It prefers to break at the end of a sentence ('.', '!', '?'), then on whitespace, and only hard-cuts on a weighted-unit boundary as a last resort." },
        { q: "Do the 1/N markers count toward the limit?", a: "Yes — the splitter reserves space for the worst-case marker length and then re-calculates the budget using the true total post count, so no piece ever exceeds the limit." },
        { q: "Can I disable the numbering?", a: "Yes. Uncheck 'Add 1/N numbering' to remove the markers entirely, or clear the marker field to drop just the emoji prefix." },
        { q: "Does the emoji marker affect my count?", a: "Yes — emoji count as 2 weighted units (per X's rules), so adding 🧵 to every piece costs 2 chars per post. The budget reflects that automatically." },
        { q: "Will my draft be saved?", a: "Yes, locally. Your draft, tier choice and marker preference are stored in localStorage on this device only — Toollyz has no backend." },
        { q: "Are @mentions and #hashtags counted differently?", a: "No — they count as their literal weighted length. The counter highlights them separately so you can see what handles will be linked when posted." },
        { q: "What if my text contains only emoji?", a: "Each pictographic emoji counts as 2 weighted units, so the 280-char limit becomes roughly 140 emoji. The gauge handles this correctly because we run the same weighting at the code-point level." },
        { q: "Is my text uploaded anywhere?", a: "No. The weighting, splitting and preview happen entirely in your browser. Toollyz has no server that sees what you type." },
      ],
    },
  },
  {
    ...
    seo: {
      ...
      relatedSlugs: ["social-post-formatter", "instagram-caption-formatter", "fancy-text-generator", "tweet-character-counter"],
      faqs: [
        { q: "Why bold only the hook?", a: "LinkedIn collapses everything after ~210 characters into '…see more'. A bold hook stops the scroll while the body stays readable in normal text. Bolding the whole post often reads as shouty." },
        { q: "What's the 210-character rule?", a: "LinkedIn's mobile feed shows ~210 characters before truncating with '…see more'. Posts that resolve the curiosity gap before the cutoff get clicked through more reliably." },
        { q: "Does the bold trick work on the LinkedIn app?", a: "Yes. We use Unicode Mathematical-Alphanumeric characters (𝐁𝐨𝐥𝐝, not <b>Bold</b>) so the formatting is part of the text and survives copy-paste into the LinkedIn web composer and mobile apps." },
        { q: "Are there accessibility concerns with the bold trick?", a: "Yes — Unicode bold characters aren't always read correctly by screen readers, which may announce them as math symbols. For accessibility-first audiences, leave the hook plain and use a strong opening word instead." },
        { q: "What does the hook scorer actually check?", a: "5 heuristic best-practice checks: 1) hook ≤ 210 chars, 2) asks a question or makes a claim, 3) contains a number, 4) at most 1 emoji, 5) uses 'you' / 'your' to talk to the reader. Higher score correlates with higher LinkedIn engagement in published studies." },
        { q: "Why 3–5 hashtags, not more?", a: "LinkedIn's algorithm rewards 3–5 niche hashtags far more than long hashtag walls. We warn at 10+. There's no hard penalty, but the diminishing-returns curve is steep." },
        { q: "Does the blank-line trick break accessibility?", a: "U+2063 INVISIBLE SEPARATOR is ignored by screen readers so the audio output is unchanged. Visual rendering gets the gap — the standard fix used by every social-scheduling tool." },
        { q: "Can I use this for LinkedIn newsletters?", a: "Yes for the body, but newsletters use proper rich-text formatting in the LinkedIn newsletter composer. The Unicode bold here is for posts that paste into the regular share box." },
        { q: "What's the character limit on LinkedIn?", a: "3 000 characters for a regular post. Articles and newsletters have no practical limit. Toollyz warns when you cross 3 000." },
        { q: "Is my draft saved anywhere?", a: "Only locally — your draft, style choices and hashtag block save to localStorage on this device. Toollyz has no backend." },
        { q: "Will the formatting work on Sales Navigator or LinkedIn ads?", a: "The Unicode bold characters render identically. Some ad formats may flag heavily-formatted text as spammy — use the bold-hook variant for organic posts and keep ads plain." },
      ],
    },
  },
  {
    ...
    seo: {
      ...
      relatedSlugs: ["linkedin-post-formatter", "social-post-formatter", "hashtag-generator", "tweet-character-counter"],
      faqs: [
        { q: "Why does Instagram swallow my line breaks?", a: "Instagram's mobile composer collapses standalone empty lines on render. Inserting an invisible character on each empty line gives the line content — so the renderer treats it as a 'short paragraph' and keeps the gap." },
        { q: "What's the 125-character cut-off?", a: "In the Instagram feed, the visible caption snippet is roughly 125 characters before '…more' appears. Anything past that is hidden until tapped. The hook preview at the top of this tool shows you what readers see before tapping." },
        { q: "Why hide hashtags below the &quot;more&quot; fold?", a: "Visible hashtag blocks look cluttered and reduce engagement on the feed preview. Pushing them past the fold means the visible snippet is your message; the hashtags only appear once a reader taps to expand." },
        { q: "How many spacer lines should I use?", a: "5 is the standard. Fewer can still show a hashtag tail; more wastes character budget. The tool defaults to 5 and lets you tune 0–8." },
        { q: "Why 5–11 hashtags?", a: "Multiple Instagram studies (and Meta's own engineering blog) point to 5–11 niche hashtags as the discovery sweet spot. Below 5 cuts reach; above 11 reads as spammy and triggers downranking on some account tiers." },
        { q: "Does U+2063 cause shadowban issues?", a: "There's no public evidence that U+2063 (INVISIBLE SEPARATOR) triggers Meta's anti-spam systems — it's the standard fix used by every commercial scheduling tool (Later, Buffer, Planoly). U+200B (ZERO WIDTH SPACE), by contrast, has historically been flagged; that's why we avoid it." },
        { q: "Will the bold trick work in Stories or Reels?", a: "In captions, yes. Story text uses its own renderer that doesn't always honour Unicode bold. Reels captions render the same as feed captions." },
        { q: "Can I bold only part of the body?", a: "Not in this tool — the body style applies to everything. For mixed styling, paste the bold part separately or split into a multi-line carousel." },
        { q: "What's the max caption length?", a: "Instagram caps captions at 2 200 characters. The counter goes red past that limit; nothing past it will save." },
        { q: "Does this work on Threads or Facebook?", a: "Use the Social Post Formatter for Threads / Facebook — the line-break trick is the same, but the 125-char fold and 5–11 hashtag rules are Instagram-specific." },
        { q: "Is anything uploaded?", a: "No. The whole tool runs in your browser." },
      ],
    },
  },
  {
    ...
    seo: {
      ...
      faqs: [
        ...
        { q: "Which format is the 'default'?", a: "If you write `<t:UNIX>` without a format code, Discord uses the short date/time format ('f'). Toollyz marks that card with a 'default' badge." },
        { q: "Why is the relative format useful?", a: "It updates live in every viewer's client — 'in 5 minutes', 'in 2 hours', '3 days ago' — perfect for event announcements that should feel timely no matter when they're read." },
        { q: "Will the timestamp work in all Discord clients?", a: "Yes — desktop, web, mobile and Discord-embeds all parse the syntax the same way. Some bots may strip it if they re-emit your message; the bare syntax itself works everywhere." },
        { q: "What if I want to schedule a message?", a: "Discord timestamps don't trigger scheduled posts on their own — they just localise the display. Pair them with a Discord bot or scheduling tool to send a message at the right moment, including a Toollyz-generated `<t:R>` countdown." },
        { q: "How precise can I be?", a: "Discord timestamps are in seconds, not milliseconds. The Unix seconds field lets you pick a precise second; the calendar input rounds to the nearest minute by default." },
        { q: "Does timezone selection matter?", a: "You pick the moment using your local timezone (or the absolute Unix seconds). Discord then renders that moment in each viewer's local timezone automatically — no manual TZ conversion needed." },
        { q: "Why are the preview times in MY locale?", a: "The preview uses your browser's Intl.DateTimeFormat with your locale and timezone. Discord does the same for each viewer — so the preview shows you what your viewers in the same locale will see." },
        { q: "Will copying the code also copy the rendered text?", a: "We copy the raw `<t:UNIX:code>` syntax — that's what Discord needs to localise it for each viewer. Pasting it as plain text would defeat the purpose." },
        { q: "Is anything uploaded?", a: "No. All timestamp generation and localisation previewing run entirely in your browser." },
      ],
    },
  },
  {
    ...
    seo: {
      ...
      faqs: [
        ...
        { q: "Why are there three URL flavours?", a: "wa.me is the universal short URL (works on every platform). api.whatsapp.com/send/ opens WhatsApp Web in browsers more reliably. whatsapp:// is a native deep link that skips the browser entirely on mobile." },
        { q: "Will it work without the recipient saving my number?", a: "Yes — that's the whole point. The link opens a chat with the number directly; no contact-save required." },
        { q: "Is there a character limit on the pre-filled message?", a: "WhatsApp itself doesn't enforce one, but very long URL-encoded messages (over ~2000 encoded chars) can fail on older iOS Safari. Toollyz warns when you cross that line." },
        { q: "Can I scan the QR with WhatsApp directly?", a: "Yes. Any camera or QR scanner opens the link, which then jumps into WhatsApp. The QR is generated client-side and downloadable as a PNG." },
        { q: "Does this support WhatsApp Business?", a: "Yes — wa.me links work identically for WhatsApp Business numbers. The generated link opens whichever WhatsApp app the recipient has installed." },
        { q: "Will my phone number be stored?", a: "No. All link generation and QR rendering happens entirely in your browser. Toollyz has no backend that sees the phone number or the message." },
        { q: "Why is my calling code listed wrong?", a: "Some countries share calling codes (the US and Canada both use +1, for example). Manually edit the calling-code field if the picker isn't precise enough." },
        { q: "Can I use this for support / sales pages?", a: "Yes — that's the most common use. Embed the generated URL in a button or anchor on your site so visitors can start a chat with one click." },
        { q: "Does the QR include the pre-filled message?", a: "Yes. The QR encodes the full URL including the encoded message, so scanning opens the chat with the message already typed." },
      ],
    },
  },
  ...
  // ─── DEVELOPER (batch 2) ─────────────────────────────────────────────────
  {
    ...
    seo: {
      ...
      faqs: [
        ...
        { q: "What does 'ignore whitespace' do?", a: "It collapses runs of spaces and tabs into a single space and trims each line before comparing — so a line where you only re-indented doesn't show as a change." },
        { q: "What does 'pair add and remove as change' do?", a: "When a run of removed lines is followed by a run of added lines, the tool pairs them up into single 'change' rows side-by-side. This is much easier to read when you've rewritten a paragraph rather than inserted or deleted whole lines." },
        { q: "How big can the inputs be?", a: "The LCS table is m × n integers, so memory use grows quadratically. In a browser, a few thousand lines per side is comfortable; tens of thousands may slow down." },
        { q: "What is the similarity percentage?", a: "It's the number of unchanged lines divided by the larger of the two inputs' line counts — a rough but useful indicator of how much the two texts overlap." },
        { q: "Can I download a patch file?", a: "Yes. The 'Download .patch' button saves a unified-style patch (lines prefixed with +, - or two spaces) that you can hand to anyone for review." },
        { q: "Why are some changed rows shown as separate add/remove instead of as a 'change'?", a: "Toollyz pairs adds and removes only when they sit immediately next to each other. Leftovers — like 3 removes followed by 1 add — pair the first one and leave the rest as pure removes." },
        { q: "Does it work on code?", a: "Yes. Use 'ignore whitespace' if formatting was the only change; leave it off if you care about indentation differences." },
        { q: "Is my text uploaded anywhere?", a: "No. The diff runs entirely in your browser — Toollyz has no server that ever sees your text." },
        { q: "Can I compare files instead of pasting?", a: "Not directly yet, but you can drag the file's contents into the text area, or open it in any editor and copy-paste both sides in." },
      ],
    },
  },
  {
    ...
    seo: {
      title: "Duplicate Line Remover — Dedupe Lists Online",
      description:
        "Remove duplicate lines from any text or list with one click. Ignore-case, trim-whitespace, drop-blanks and live frequency table. Runs entirely in your browser.",
      what:
      ...
      faqs: [
        ...
        { q: "Does it remove duplicates that appear out of order?", a: "Yes. Order doesn't matter for detection — a duplicate is any line whose normalised form matches a line seen earlier anywhere in the input." },
        { q: "What's the frequency table for?", a: "It shows every unique line and how many times it appeared in the input, ranked by count. Useful for finding the most-repeated entries when a CSV got mis-merged." },
        { q: "Can I remove blank lines too?", a: "Yes. The 'drop blank lines' toggle skips empty lines entirely — they won't appear in the output or the frequency table." },
        { q: "Does it work on huge lists?", a: "Yes. The implementation uses a Set for O(n) detection; we routinely test 100 000-line inputs in the browser without lag." },
        { q: "Will it break CSVs or quoted strings?", a: "No — the tool only splits on newline characters. Each CSV row is treated as a whole line, including any commas, quotes or embedded escapes." },
        { q: "Can I sort the result alphabetically?", a: "Yes — switch the order toggle to 'Sort A → Z' and the unique lines are alphabetised before output." },
        { q: "Does the trailing newline matter?", a: "No. We strip a single trailing empty line before splitting so a final-newline file doesn't pick up a phantom blank entry." },
        { q: "Is my list uploaded anywhere?", a: "No. The dedupe is a pure browser function — Toollyz has no server that ever sees your list." },
        { q: "Does this work on emojis or non-ASCII text?", a: "Yes. Comparison is by string equality after the chosen normalisations, so Unicode lines compare correctly." },
      ],
    },
  },
  {
    ...
    seo: {
      ...
      how: [
        "Paste your list (one item per line) into the input panel.",
        "Pick a sort method — A→Z, natural, by number, by length, shuffle or reverse.",
        "Toggle case-sensitive, ignore-leading-spaces, drop-blank-lines and dedupe-after-sort as needed.",
        "Copy the sorted output, download as .txt, or feed it back as the new input.",
      ],
      benefits: [
      ...
      faqs: [
        ...
        { q: "Is the shuffle truly random?", a: "Yes — Toollyz uses `crypto.getRandomValues` to seed a Fisher-Yates shuffle, which is the standard cryptographically-strong shuffle algorithm. On platforms without the WebCrypto API it falls back to Math.random." },
        { q: "What does 'ignore leading spaces' do?", a: "It strips leading whitespace from each line before comparing, so an indented list like a code outline sorts on the content rather than the indentation level." },
        { q: "What's the difference between case-sensitive and case-insensitive?", a: "Case-sensitive sort uses 'variant' collation (Apple, apple are distinct). Case-insensitive uses 'base' collation so they sort together — typical for human-readable lists." },
        { q: "Will it sort emoji and non-Latin text correctly?", a: "Yes. The standard alpha modes use Intl.Collator with locale-aware Unicode comparison, so accents and non-Latin scripts collate in the order your browser's locale expects." },
        { q: "Can I run shuffle twice to get a different order?", a: "Yes — click the shuffle button again and the seed bumps, producing a fresh random order." },
        { q: "Does it preserve duplicates by default?", a: "Yes. Sorting does not remove duplicates unless you tick 'Dedupe after sorting'." },
        { q: "What does 'reverse' do?", a: "It simply flips top↔bottom — no sort. Useful for inverting an already-sorted list." },
        { q: "Does this run on the server?", a: "No. Sorting is a pure browser computation — Toollyz has no backend that sees your list." },
        { q: "How big a list can I sort?", a: "Comfortable up to hundreds of thousands of lines. The bottleneck is browser memory rather than the algorithm itself." },
      ],
    },
  },
  {
    ...
    seo: {
      ...
      how: [
        "Pick a file with the file-picker or drag-and-drop into the panel.",
        "Choose a shred pattern — DoD 3-pass (default), single random, zero-fill or one-fill.",
        "Click 'Shred' and watch the progress bar tick through each pass.",
        "Download the shredded file and (optionally) drag it over the original on your disk.",
      ],
      benefits: [
      ...
      faqs: [
        ...
        { q: "When would I actually use this?", a: "(1) Confirming an upload pipeline copies bytes — shred a known-content file, upload, verify the backend sees random bytes. (2) Sharing a file's metadata (name + size) without the contents — say, demonstrating a workflow without exposing real customer data. (3) Personal-data sanitisation when you have to send a placeholder of a given file size." },
        { q: "Why is there a 50 MB cap?", a: "Browsers allocate a Uint8Array of the file's size in memory. 50 MB is a comfortable ceiling on phones and laptops; bigger files risk OOM crashes in the browser tab. For larger jobs, use a native tool like `shred -u` on Linux." },
        { q: "Is the random source strong enough?", a: "Yes — we call `crypto.getRandomValues`, which is the WebCrypto cryptographically-strong RNG. The implementation is platform-native (BoringSSL on Chrome, etc.) and seeded from the OS entropy pool." },
        { q: "Why chunk into 64 KB blocks?", a: "`crypto.getRandomValues` caps at 65 536 bytes per call. We iterate the buffer in 64 KB slices and fill each independently. The same code path handles 1 KB and 50 MB inputs uniformly." },
        { q: "What does the SHA-256 receipt prove?", a: "It proves the byte sequence of the shredded download — useful when you want to attest later that a particular file was written. Don't rely on it for tamper-evidence after the fact; once the file leaves your browser, its hash can change." },
        { q: "Will this work on flash / SSD storage?", a: "Multi-pass overwrites are designed for magnetic media. Flash and SSD storage controllers do wear-levelling, so the original blocks may still exist physically. For SSD sanitisation use the drive's built-in Secure Erase command, not this tool." },
        { q: "Does the original filename stay the same?", a: "We suggest a `<name>.shredded.<ext>` filename for the download. You can rename it anything when saving." },
        { q: "Does my file leave my browser?", a: "No. The file is read with FileReader, processed in-memory, and downloaded back to you via a blob URL. Toollyz has no server that ever sees a byte of the file." },
        { q: "Can it shred multiple files at once?", a: "Not yet — the current version handles one file at a time. Drag-and-drop another file when you're done with the first." },
      ],
    },
  },
  {
    ...
    seo: {
      ...
      how: [
        "Pick an encoding — hex / base64 / base64url / alphanumeric.",
        "Set the desired token length in characters and how many tokens to mint.",
        "Optionally add a prefix like `sess_`.",
        "Click Regenerate, copy individual tokens or download the whole batch as .txt.",
      ],
      benefits: [
      ...
      faqs: [
        ...
        { q: "Why does the alphanumeric mode use 'rejection sampling'?", a: "If you took a random byte (0-255) and did `byte % 62` you'd over-represent the first 8 characters of the alphabet because 256 isn't a multiple of 62. We reject bytes ≥ 248 and only use the rest, so every character has exactly 1/62 probability." },
        { q: "What's a 'good' token length?", a: "For session cookies, 128 bits of entropy is the consensus minimum and 256 bits is the standard for serious deployments. Hex needs 32 / 64 characters; Base64URL needs 22 / 43; alphanumeric needs ~22 / 43 — the strength meter shows you live." },
        { q: "Can I use these as API keys?", a: "Yes, but our API Key Generator is purpose-built with prefix, segmented blocks and checksum support — better suited for keys that humans paste around." },
        { q: "Can I generate millions of tokens?", a: "Each click mints up to 500. For more, click Regenerate again — there's no rate limit because everything's local. A 500-token batch of 32-char tokens takes a few milliseconds." },
        { q: "How does the strength meter calculate bits?", a: "Bits = `floor(length × log2(alphabet_size))`. Hex's alphabet is 16 chars (4 bits/char); base64 / base64url have 64 chars (6 bits/char); alphanumeric has 62 chars (~5.95 bits/char)." },
        { q: "Is Math.random ever used?", a: "Only as a last-resort fallback if `crypto.getRandomValues` is somehow unavailable — which would only happen in an extremely old browser. In practice every modern browser supports it." },
        { q: "Are my tokens stored?", a: "Only your last settings persist (encoding, length, prefix, count). The generated tokens themselves live only in the page state — refresh and they're gone." },
        { q: "Why is there a Cookie icon?", a: "Session IDs are most commonly stored in HTTP cookies — the icon is a nod to that. The tool works just as well for any random-token use case." },
        { q: "Is anything uploaded?", a: "No. Token minting, the strength calculation and the entropy estimation all run in your browser. Toollyz has no backend." },
      ],
    },
  },
  {
    ...
    seo: {
      ...
      faqs: [
        ...
        { q: "What's the checksum character for?", a: "It's a single sum-mod-36 character we append after the body. If a user mistypes one character of the key, the checksum almost certainly won't match — useful to fail fast client-side before hitting your auth endpoint. It's not cryptographic." },
        { q: "Why are 'segments' useful?", a: "Long uninterrupted strings are hard to dictate over a phone or read off a slide. Splitting into 4×6 or 5×6 with a `-` separator makes keys human-friendly without sacrificing entropy." },
        { q: "What charset should I pick?", a: "Alphanumeric (a-z A-Z 0-9) is the default and works everywhere. Hex is best when keys need to be all digits/lower for case-insensitive systems. Base64URL packs more entropy per character — use it when key length matters." },
        { q: "Can I generate API keys offline?", a: "Yes — that's the point. Everything happens in the browser; no AI service, no API, no network call. You can disconnect from the internet and the tool still works." },
        { q: "How long should an API key be?", a: "For production traffic, target ≥ 128 bits of entropy — about 22 base64url chars or 32 alphanumeric chars. The strength meter shows live. Stripe-style 32-char alphanumeric keys are ~190 bits." },
        { q: "Are the keys safe to commit?", a: "Never. Even though we generate them locally, treat them like any production secret — store in a secret manager, environment variable, or a vault. Don't put them in git." },
        { q: "Can I add my own template?", a: "Not via the UI today. Use the custom settings panel to match your format, then save it as the default in localStorage by leaving the settings active." },
        { q: "Why isn't the checksum cryptographic?", a: "It's a one-character typo detector — there are only 36 possible values, so an attacker who knows the algorithm can trivially compute it. The point is catching paste errors, not preventing forgery." },
        { q: "Is anything uploaded?", a: "No. All key minting and strength calculation runs in your browser." },
      ],
    },
  },
  {
    ...
    seo: {
      ...
      how: [
        "Pick an algorithm — AES-GCM is the safe default for new symmetric work.",
        "Pick the key length (AES, RSA, HMAC) or curve (ECDSA, ECDH).",
        "Toggle extractable off for production use, on for export to JWK / PEM.",
        "Copy or download the key material — private keys are hidden behind a reveal toggle.",
      ],
      benefits: [
      ...
      faqs: [
        ...
        { q: "Why RSA-PSS instead of RSA PKCS#1 v1.5?", a: "PSS is probabilistic (different ciphertext each time) and has stronger security proofs. PKCS#1 v1.5 is still widely deployed (every RS256 JWT) but new code should prefer PSS." },
        { q: "Why is my private key hidden by default?", a: "Private keys are the most sensitive output. Hiding them behind a reveal toggle prevents accidental leakage via screen recording, screen-share, or browser session captures." },
        { q: "What does 'non-extractable' mean?", a: "The Web Crypto API returns a CryptoKey object that points to key material the browser tracks internally. With extractable=true you can export the bytes (raw, JWK, PEM); with extractable=false you can't, and the key can only be used via crypto.subtle.encrypt/decrypt/sign/verify — much safer for production." },
        { q: "Can I use these for SSL / TLS?", a: "RSA / ECDSA keys generated here are structurally identical to what openssl produces. To use for TLS, export the PEM and feed it to your server config along with a certificate signed for your domain (via a CA or Let's Encrypt)." },
        { q: "Why P-256 vs P-384?", a: "P-256 (also called secp256r1) gives ~128-bit security and is the fastest of the NIST curves. P-384 gives 192-bit security. P-521 gives 256-bit. Most TLS today uses P-256." },
        { q: "Are AES-128 keys still safe?", a: "Yes for current threat models. AES-256 buys you margin against future quantum attacks but is slower. AES-128 is the spec default and what most browsers/protocols negotiate." },
        { q: "Why no Ed25519?", a: "The Web Crypto API doesn't support Ed25519 in all browsers yet (Safari is the laggard). When support is universal we'll add it. For now, ECDSA on P-256 is the safe portable choice." },
        { q: "Is anything uploaded?", a: "No. All key generation runs in your browser via crypto.subtle — Toollyz has no server that ever sees the key material." },
        { q: "Will the same configuration generate the same key?", a: "No — every call to crypto.subtle.generateKey produces a fresh random key. The strength meter shows the configuration; the bytes change every time." },
      ],
    },
  },
  {
    ...
    seo: {
      ...
      how: [
        "Edit the claims JSON — start with the sample (`iss`, `sub`, `iat`, `exp`, `name`, `scope`) or paste your own.",
        "Pick the HMAC algorithm (HS256 default; HS384 / HS512 for stronger digests).",
        "Set the shared secret. Tick 'secret is base64url-encoded raw bytes' if your key is binary.",
        "Copy the signed JWT or any of its three segments individually for inspection.",
      ],
      benefits: [
      ...
      faqs: [
        ...
        { q: "Is my secret sent anywhere?", a: "No. The signing happens via `crypto.subtle.sign` entirely in your browser. Toollyz has no server that ever sees your secret or the claims." },
        { q: "What's the difference between 'secret as text' and 'base64url raw bytes'?", a: "Most JWT examples use a UTF-8 string as the secret. But the spec says the HMAC key is raw bytes — if you generated a 32-byte random key with `openssl rand -base64url 32`, tick the box so we decode the base64url back to bytes before signing." },
        { q: "Does it verify existing JWTs?", a: "No — this tool only signs. To verify a JWT, use the companion JWT Decoder tool, which displays the header, payload and signature so you can re-sign here with the same secret and compare." },
        { q: "Why is the same input producing different JWTs sometimes?", a: "It shouldn't — claims are stably stringified with sorted keys before signing, so the same input always yields the same JWT. If you see drift, check whether `iat` is being set to `Math.floor(Date.now() / 1000)` and pinning that value." },
        { q: "What's `iat` vs `exp` vs `nbf`?", a: "`iat` is when the token was minted, `nbf` is the earliest time it's valid, `exp` is when it expires — all in UNIX seconds. Most APIs care about `exp` (must be in the future) and sometimes `nbf` (must be in the past)." },
        { q: "Does the algorithm in the header matter?", a: "Yes — we always set the `alg` field of the header to match the selected algorithm. If you edit the header JSON to claim a different `alg`, signing still uses what's selected here, so the result would fail verification." },
        { q: "Will this work with auth0 / firebase / supabase?", a: "Yes if they accept HS256/HS384/HS512 tokens. Most platforms use RS256 with platform-issued keys, so generated tokens here won't verify against their public keys." },
        { q: "Can I generate keys for the secret?", a: "Use the Session ID Generator or API Key Generator with the base64url encoding and 32-byte body length — that produces a strong HS256 key. Then paste it here with the base64url toggle on." },
        { q: "Is anything stored?", a: "Only locally — your last claims, algorithm, secret choice and base64url flag are persisted in localStorage on this device. Toollyz has no backend." },
      ],
    },
  },
  {
    ...
    seo: {
      ...
      faqs: [
        ...
        { q: "Does it handle `*/5`, `1-30/5`, `0,15,30,45`?", a: "Yes — all standard cron syntax: ranges (`1-30`), steps (`*/5`, `1-30/5`), lists (`0,15,30,45`) and bare values are all supported. Name-style months (`jan-jun`) and weekdays (`mon-fri`) also parse." },
        { q: "What timezone are the next runs shown in?", a: "Your local browser timezone, as reported by `Intl.DateTimeFormat().resolvedOptions().timeZone`. The relative deltas ('in 12 min') use `Date.now()` so they stay honest as time passes." },
        { q: "What if my cron never fires (e.g. Feb 30)?", a: "We cap forward iteration at 4 years. If no matching date is found within that window, the next-runs list comes back empty and we surface a warning." },
        { q: "Does it support special characters like `L` (last) or `#` (nth weekday)?", a: "Not yet — those are Quartz extensions and we're focused on POSIX cron. Most schedulers (Linux crontab, Vixie cron, GitHub Actions cron) don't support them either." },
        { q: "Does it differentiate cron implementations?", a: "We aim for POSIX cron behaviour. Most Linux schedulers and CI cron schedulers (GitHub Actions, GitLab, Render) all follow this. Quartz-specific syntax may parse but not behave identically." },
        { q: "What about `?` in day-of-month or day-of-week?", a: "Quartz uses `?` to mean 'no specific value'. POSIX cron doesn't recognise it. We currently reject `?`; use `*` instead." },
        { q: "Will it tell me if my cron is too aggressive?", a: "Not explicitly — but the next-runs view makes it obvious: if you see 8 firings within 60 seconds, your cron will probably overlap itself in production." },
        { q: "Is anything uploaded?", a: "No. Parsing, English description and run computation are all pure browser functions." },
        { q: "Can it convert from English to cron?", a: "That's what the Cron Job Expression Generator does — visual builder that produces a valid cron expression. The two tools cross-link." },
      ],
    },
  },
  {
    ...
    seo: {
      ...
      how: [
        "For each of the 5 fields, pick a mode: every, list, range, or step.",
        "Toggle values (clickable buttons for months and weekdays) or type ranges and steps.",
        "Watch the cron expression and English description update live in the hero.",
        "Browse the next 5 firing times in your local timezone, then copy the expression.",
      ],
      benefits: [
      ...
      relatedSlugs: ["cron-time-translator", "stopwatch", "alarm-clock", "world-clock"],
      faqs: [
        { q: "What's the difference between this and the Cron Time Translator?", a: "Translator goes English ← expression — paste cron, read the breakdown. Generator goes expression ← visual — pick UI options, get cron. They share the same parser so the round-trip is consistent." },
        { q: "Which cron flavour does this build?", a: "Standard 5-field POSIX cron (`minute hour day-of-month month day-of-week`). Compatible with Linux crontab, Vixie cron, GitHub Actions cron, GitLab CI cron, Render and most other schedulers." },
        { q: "How do I pick 'every weekday at 9 AM'?", a: "Minute: list with '0'. Hour: list with '9'. Day-of-month: every. Month: every. Day-of-week: list with Mon-Fri selected. The result is `0 9 * * 1-5` (or `0 9 * * 1,2,3,4,5` depending on selection — both work)." },
        { q: "How do I pick 'every 5 minutes'?", a: "Minute: step → 5. Everything else: every. The result is `*/5 * * * *`." },
        { q: "What's a 'range'?", a: "An inclusive range of values, e.g. hours 9-17 means 9 AM through 5 PM. Useful for office hours." },
        { q: "Can I have a step inside a range?", a: "Not in this UI — pick 'list' and type the comma-separated values manually for arbitrary patterns. Or use the Cron Time Translator to validate any expression you craft." },
        { q: "Will the day-of-month and day-of-week interact the way I expect?", a: "Standard cron: when both are non-`*`, EITHER match fires (union). So 'day-of-month=13 AND day-of-week=Fri' fires on every Friday OR every 13th. The preview makes this obvious." },
        { q: "Does the preview match what Linux crontab will do?", a: "Yes for the standard cases. We follow POSIX semantics including the DOM/DOW union rule. Quartz-style schedulers (which use intersection) may differ — though most do follow POSIX." },
        { q: "What timezone are the next runs?", a: "Your local browser timezone. Most cron daemons run in the server's timezone — check before relying on the preview for production scheduling." },
        { q: "Is anything uploaded?", a: "No. Building the expression and computing next runs are pure browser functions." },
        { q: "Can I save my schedule?", a: "Your builder state persists in localStorage on this device. To share with a teammate, just copy the cron expression itself." },
      ],
    },
  },
  {
    ...
    seo: {
      ...
      how: [
        "Browse the library by category (Common / Web / Identifiers / Numbers / Dates) or search.",
        "Or switch to Visual builder, pick a character class, quantifier and flags.",
        "Read the plain-English explanation of what your regex matches.",
        "Edit the pattern in the live tester, paste a test input, and verify the matches.",
      ],
      benefits: [
      ...
      faqs: [
        ...
        { q: "What does the visual builder actually do?", a: "It composes one character class with one quantifier, optionally anchored, with chosen flags. It's deliberately simple — for multi-component regex (alternation, lookahead) use the library or write the pattern by hand." },
        { q: "Why no lookahead / lookbehind in the builder?", a: "Lookahead and lookbehind have subtle gotchas and can produce hard-to-debug regex when composed visually. The library includes lookahead-based patterns (strong password) where they're genuinely useful." },
        { q: "What flags should I use?", a: "g (global) is the default for 'find all'. i is case-insensitive. m treats `^` and `$` as start/end of LINE instead of string. s (dot-all) makes `.` match newlines too. u is for Unicode-aware matching." },
        { q: "Will my test input be saved?", a: "Yes — locally. The pattern, flags and test input all persist in localStorage on this device for one-click resume." },
        { q: "Is my regex executed against arbitrary input?", a: "Yes, but inside a try/catch with a 1000-match safety cap. A pathological 'catastrophic backtracking' regex might slow your browser but can't escape it." },
        { q: "Why does my regex say 'invalid'?", a: "Most often: unescaped special characters ( ( [ { . ? * + | ^ $ \\ ) or a flag set the engine doesn't recognise. The error message under the input gives the exact reason." },
        { q: "Can I get the regex in PCRE / Perl / Python flavour?", a: "The patterns shown use JavaScript regex syntax (ECMAScript). Most patterns translate directly to PCRE / Perl / Python; the main differences are lookbehind (Python yes, JS yes since ES2018) and Unicode property escapes." },
        { q: "What's the difference between hex / colon-separated MAC formats?", a: "We provide the colon/dash form. For Cisco dot-form (AABB.CCDD.EEFF) use the MAC Address Lookup tool — it accepts every form." },
        { q: "Is anything uploaded?", a: "No. Pattern building and testing run entirely in your browser." },
      ],
    },
  },
  {
    ...
    seo: {
      ...
      how: [
        "Paste an unindented or messy SQL query — single-statement or multi-statement.",
        "Toggle UPPERCASE keywords (default on) and pick the indent width (2–8 spaces).",
        "Read the per-token stats — keywords, identifiers, strings, statements.",
      ...
      faqs: [
        ...
        { q: "Does it handle multi-statement input?", a: "Yes. Each `;` ends a statement and we keep them visually separated. The token statistics count multiple statements." },
        { q: "Will it break inside string literals?", a: "No. Single-quoted strings (with `''` for embedded quote) are recognised by the tokeniser and passed through unchanged." },
        { q: "Why are keywords UPPERCASE by default?", a: "It's the SQL standard convention and improves readability — identifiers stay lower-case, keywords stand out. Turn off the toggle if your house style is all-lower." },
        { q: "Does it preserve my comments?", a: "Yes — both `--` line comments and `/* */` block comments. Line comments break to a new line; block comments stay where they are." },
        { q: "How does it handle CTEs?", a: "`WITH foo AS (…)` formats with the CTE name on its own indented line, the body inside the parens at the next depth, and the main SELECT after — same pattern Postgres docs use." },
        { q: "What about window functions like ROW_NUMBER() OVER (PARTITION BY …)?", a: "PARTITION BY is in the multi-word clause list, so it stays together. The OVER clause renders inline with the function call." },
        { q: "Will it work on tens of thousands of lines?", a: "The tokenizer is O(n), so yes — large migrations or fixture files format fine. The browser's textarea is the practical limit." },
        { q: "Can it format MySQL CREATE PROCEDURE bodies?", a: "Partially — the IF / WHILE / LOOP keywords aren't in the top-clause list, so they're rendered inline. For procedure bodies you may want a dialect-specific tool." },
        { q: "Is anything uploaded?", a: "No. Tokenising and re-indenting are pure browser computations." },
      ],
    },
  },
  {
  ...
  },
  {
    ...
    seo: {
      ...
      how: [
        "Paste your TOML — Cargo.toml, pyproject.toml, anything 1.0-compliant.",
        "Toggle 'sort keys alphabetically' or change the array-break threshold as you like.",
        "Read the line-pointing error message if the parse fails.",
        "Copy the formatted output or download as .toml.",
      ],
      benefits: [
      ...
      faqs: [
        ...
        { q: "What about pyproject.toml?", a: "Yes. `[tool.poetry]`, `[build-system]`, `[project]`, `[tool.ruff.lint]` and friends all parse and re-emit." },
        { q: "Can it convert TOML to JSON?", a: "Not in this version — the focus is formatting and validation. If you need TOML → JSON, paste the formatted TOML into a server-side converter; the predictable formatting makes that easier." },
        { q: "How does it handle multi-line arrays?", a: "Inline by default. When the array has ≥ the 'Array break at' threshold items (default 4), we break each onto its own line for readability. Tune the threshold to taste." },
        { q: "Will key sort break my Cargo.toml?", a: "Cargo doesn't care about key order — it's safe to sort. But the alphabetical sort is OFF by default since some configs (eslint, prettier) read better with logical grouping intact." },
        { q: "What's a 'dotted key'?", a: "TOML lets you write nested keys as `tool.poetry.dependencies = { … }` or split the same intent into nested tables. Dotted keys are parsed as nested tables — when you re-emit, you get the table form." },
        { q: "Does it handle multi-line strings correctly?", a: "Yes — both basic (`\"\"\"`) and literal (`'''`) multi-line strings. The leading newline immediately after the opening delimiter is stripped per spec." },
        { q: "What about RFC 3339 datetimes?", a: "Preserved verbatim. We detect them with a regex and pass them through as opaque strings — re-formatting dates would require timezone handling that's out of scope." },
        { q: "Is anything uploaded?", a: "No. The whole parser and emitter run in your browser." },
        { q: "Will it warn me about duplicate keys?", a: "TOML 1.0 says duplicate keys are an error. We currently overwrite silently — a future version will flag duplicates explicitly." },
      ],
    },
  },
  {
  ...
  },
  {
    ...
    seo: {
      ...
      how: [
        "Browse by section: Bash/Zsh, Git, Docker, Network, macOS, Linux.",
        "Or search by command name, flag or what you're trying to do.",
        "Click any tag chip (#file, #log, #git, …) to narrow further.",
      ...
      faqs: [
        ...
        { q: "Are the examples tested?", a: "Yes, on macOS Sonoma and Ubuntu 22.04. Some Linux-only commands (systemctl, journalctl, apt) require a Linux host." },
        { q: "Can I add my own commands?", a: "Not in the UI today. The cheat sheet is a static module — fork the repo on GitHub to contribute additions." },
        { q: "Why is `rm -rf` in here?", a: "Because everyone uses it. The example calls it on `.next` which is safe to delete. Treat the entry as a reminder of the danger." },
        { q: "What's the difference between #file and #dir tags?", a: "#file = operations on files (cp, mv, rm, cat). #dir = operations on directories (cd, mkdir, tree). #search = finders (find, grep, mdfind, lsof). The tags overlap and we lean toward listing commands in multiple tags rather than picking one." },
        { q: "Is there a print-friendly view?", a: "Not yet. Use the browser's print-to-PDF; the layout is clean enough to fit a couple of A4 pages." },
        { q: "Does it work offline?", a: "Yes — the whole cheat sheet is bundled with the page. Once loaded, no further network calls." },
        { q: "What's `rsync -av --progress` for?", a: "Mirror a local directory to a remote (or vice versa) with archive flags, verbose progress and human-readable rates. The example targets a `build/` directory upload — change the source/dest as needed." },
        { q: "Why `ss -tunlp` instead of `netstat`?", a: "ss replaces netstat on modern Linux — faster and shows more info. The flags: -t (TCP), -u (UDP), -n (numeric), -l (listening), -p (process)." },
        { q: "Is anything uploaded?", a: "No. The cheat sheet runs entirely in your browser." },
      ],
    },
  },
  {
  ...
  },
  {
    ...
    seo: {
      ...
      relatedSlugs: ["mac-address-generator", "ip-address-finder", "dns-lookup", "ping-test"],
      faqs: [
        { q: "Why isn't my MAC's vendor showing?", a: "Three possibilities: (1) the MAC's OUI isn't in our curated 300-entry table — we cover the high-traffic prefixes only; (2) the address is locally-administered (Docker, randomised iOS/Android privacy MAC, virtual NIC) so it has no IEEE-assigned vendor; (3) you typed an invalid MAC. The classification panel calls out (2)." },
        { q: "Why not ship the full IEEE OUI registry?", a: "It's ~50 000 entries and ~5 MB — a huge cost for a static page. Our curated 300+ covers Apple, Cisco, Intel, Samsung, Google, Amazon, Microsoft, HP, Dell, Lenovo, Asus, Pi, Espressif and more — which is what users actually scan in 95% of cases. We're open to PRs adding more prefixes." },
        { q: "What's the difference between universal and locally-administered?", a: "Bit 2 of the first byte. When 0, the MAC was assigned by IEEE to a real manufacturer (and we look up the OUI). When 1, the MAC was set by software — common in Docker containers (02:42:...), VPN tunnels, iOS/Android privacy MAC randomisation, virtual machines and wired NICs you manually configured." },
        { q: "What's the multicast bit?", a: "Bit 1 of the first byte. When set, the MAC is a multicast address — used by mDNS, IPv6 neighbor discovery, video streaming, gaming and some industrial protocols. FF:FF:FF:FF:FF:FF is the all-ones broadcast." },
        { q: "Why are Apple MACs sometimes randomised?", a: "iOS and macOS use 'private MAC' (randomised, locally-administered) when joining unknown Wi-Fi networks. Those addresses won't have an Apple OUI lookup — the classification panel flags them as locally-administered." },
        { q: "Does this work on IPv6 EUI-64 addresses?", a: "Partially — the EUI-64 includes a 64-bit modified MAC. Strip the middle 16 bits (FF FE) and the OUI lookup applies. For now this tool only handles 48-bit MACs." },
        { q: "Can I paste a list of MACs?", a: "Not in this version — it's single-MAC focused. For lists, use the Duplicate Line Remover to clean first then paste one at a time." },
        { q: "What's the OUI exactly?", a: "The first 24 bits (3 bytes) of a MAC address. IEEE assigns each manufacturer a unique OUI, then the manufacturer fills in the bottom 24 bits however they like." },
        { q: "Do randomised iOS MACs change every time?", a: "Per Wi-Fi network, yes — iOS picks a fresh private MAC for each SSID it joins. The randomised MAC is still locally-administered, so our classification reports it correctly." },
        { q: "Is anything uploaded?", a: "No. The OUI table is bundled with the page and the lookup is a pure browser computation." },
        { q: "How do I know if a MAC is real?", a: "If the OUI lookup returns a vendor, it's almost certainly real. If the MAC is locally-administered, it was set by software — it could be real (Docker) or fake (mac-spoof testing)." },
      ],
    },
  },
  {
    ...
    seo: {
      ...
      how: [
        "Pick Single or Bulk mode at the top.",
        "Paste a UUID (any common shape) or a list of UUIDs.",
        "Read the structural validity, version, variant and any decoded timestamp.",
        "In Bulk mode, download a TSV report of every line's status.",
      ],
      benefits: [
      ...
      relatedSlugs: ["uuid-generator", "session-id-generator", "api-key-generator", "hash-generator"],
      faqs: [
        { q: "Which UUID versions does it detect?", a: "All of them — v1 (time-based), v2 (DCE security), v3 (MD5 name-based), v4 (random), v5 (SHA-1 name-based), v6 (reordered timestamp, RFC 9562), v7 (UNIX time + random, RFC 9562) and v8 (custom, RFC 9562)." },
        { q: "What does 'variant' mean?", a: "The variant is encoded in the top bits of the 'clock-seq-hi' byte and tells you which UUID format spec was used. Modern UUIDs are 'RFC 4122 / 9562' (top bits 10). NCS / Microsoft / Reserved variants exist mostly in legacy data." },
        { q: "How does the timestamp decoding work?", a: "v1 stores a 60-bit timestamp in 100-ns intervals since 1582-10-15 (UUID epoch). v6 stores the same value but rearranged for big-endian sorting. v7 stores a 48-bit ms timestamp since 1970 — way simpler. We decode all three and show the result as ISO + relative." },
        { q: "What's a Nil UUID?", a: "The Nil UUID is all zeros (00000000-0000-0000-0000-000000000000) — RFC 4122 reserves it as a 'no UUID' placeholder. Many APIs use it for 'empty' values." },
        { q: "What's a Max UUID?", a: "Defined by RFC 9562, the Max UUID is all ones (FFFFFFFF-FFFF-FFFF-FFFF-FFFFFFFFFFFF). Symmetrical with the Nil UUID — sometimes used for 'past end' sentinels." },
        { q: "Can I paste a Microsoft GUID?", a: "Yes. We accept Microsoft GUID forms (with or without braces) and identify the variant as 'Microsoft GUID'. Structurally they're identical to RFC 4122 — only the variant byte differs." },
        { q: "Does bulk mode handle very large lists?", a: "Yes. The first 200 rows display in the table; the TSV download has every line. The parsing itself is O(n) and well-suited to thousands of UUIDs." },
        { q: "Why does v4 vs v7 matter?", a: "v4 is purely random — best for unguessability. v7 starts with a ms timestamp so UUIDs sort lexicographically by creation time — much better for database indexes. Most new projects should use v7." },
        { q: "Will it tell me which library generated my UUID?", a: "Not directly. But the version tells you broadly — Java often uses v4, Postgres often uses v4 or v7, .NET often uses v4 in 'Microsoft GUID' variant. The 'manufacturer' isn't encoded in the UUID itself." },
        { q: "Is anything uploaded?", a: "No. Parsing and version detection run entirely in your browser." },
        { q: "What if the UUID is in upper case?", a: "We normalise to lower-case before validating, so casing doesn't matter. The canonical output is lower-case per RFC 4122 recommendation." },
      ],
    },
  },
  {
  ...
  },
  {
    ...
    seo: {
      ...
      how: [
        "Paste a list of file names — one per line.",
        "Pick a rename pattern: 'name (2).ext', 'name-2.ext', 'name_2.ext' or zero-padded.",
        "Toggle case-insensitive and ignore-extension as your situation needs.",
        "Copy the suggested unique list or download a side-by-side TSV rename plan.",
      ],
      benefits: [
      ...
      faqs: [
        ...
        { q: "What does 'ignore extension' actually do?", a: "Without it, 'foo.png' and 'foo.jpg' are distinct names — no rename. With it, they're considered the same base name and one is renamed to 'foo (2).jpg' (or similar). Useful when sorting media where the extension varies but the content is the same." },
        { q: "What if my list already has 'foo (2).png' in it?", a: "The tool detects the collision and bumps the counter — your second 'foo.png' becomes 'foo (3).png' or higher until a unique name is found." },
        { q: "Can I use this with macOS/Linux `mv` to actually rename?", a: "Download the TSV, paste into a shell loop like: `while IFS=$'\\t' read orig new _; do mv -i \"$orig\" \"$new\"; done < rename-plan.tsv`. Always use `mv -i` (interactive) to confirm overwrites." },
        { q: "Does the counter start at 2 or 1?", a: "Default is 2 (the macOS/Windows convention). The first file keeps its original name; the second becomes 'name (2).ext'. Set 'Start counter at 1' to bump every duplicate." },
        { q: "What about hidden dot-files like .bashrc?", a: "We treat dot-files as having no extension. Two '.bashrc' entries collapse to '.bashrc' and '.bashrc (2)'." },
        { q: "Is this case-sensitive on Linux?", a: "Linux filesystems are case-sensitive — 'Photo.PNG' and 'photo.png' really are different files. Turn off 'case-insensitive' if your destination is Linux." },
        { q: "Will it suggest counters for dot-files like '.env'?", a: "Yes — '.env' (2) is the suggestion, which keeps the dot prefix and the lack of extension. Some filesystems treat dot-prefixed names specially; the rename plan is your guide, not a final decision." },
        { q: "Can it group by content rather than name?", a: "No — this tool sees only names. For content-based dedupe, use a tool like `fdupes` or `rmlint` that reads file contents." },
        { q: "Is anything uploaded?", a: "No. Tokenising, grouping and rename suggestions run entirely in your browser." },
      ],
    },
  },
  {
  ...
  },
  {
    ...
    seo: {
      ...
      how: [
        "Type a search query — character name, keyword, the character itself, or a U+ code point.",
        "Or browse by category — Punctuation, Math, Currency, Arrows, Shape, Latin, Greek, Cyrillic, Symbols, Emoji.",
        "Click any result to focus it in the right-hand panel.",
        "Copy the character, the code point, the HTML entity, JS escape, or CSS escape with one click.",
      ],
      benefits: [
      ...
      relatedSlugs: ["fancy-text-generator", "emoji-translator", "braille-translator", "html-minifier"],
      faqs: [
        { q: "Why 600 characters and not 150 000?", a: "The full Unicode database is ~10 MB and would slow down the page. We curate 600 high-traffic characters covering the categories users actually search for. For obscure characters, use the Unicode Character Database at unicode.org." },
        { q: "Which categories are included?", a: "Punctuation, Math, Currency, Arrows, Geometric Shape, Latin extended, Greek, Cyrillic, Symbols and Emoji. Each category has 30–80 entries." },
        { q: "Can I search by U+ code point?", a: "Yes — search for `U+2603`, `0x2603` or bare `2603` (we accept any of those forms). The exact-match lookup works for codepoints in the catalogue." },
        { q: "What if my character isn't in the catalogue?", a: "Paste it directly into the search box — exact character match works for any of the 600 entries. For characters outside the catalogue, the U+ lookup at unicode.org has every Unicode character with full metadata." },
        { q: "What's the difference between HTML hex and HTML decimal entities?", a: "Both render the same character. `&#x2603;` uses hex; `&#9731;` uses decimal (since 0x2603 = 9731). HTML supports both — hex is more readable when you can recognise the codepoint." },
        { q: "Why does the JS escape sometimes use `\\u{XXXXX}`?", a: "ECMAScript 2015 added the `\\u{HEX}` extended escape for characters above U+FFFF (the astral plane — emoji and rarely-used CJK). For codepoints ≤ U+FFFF we use the classic `\\uHHHH` format." },
        { q: "What's the CSS escape for?", a: "CSS allows Unicode escapes in selectors and content: `\\2603` (note: no curly braces — the space-or-non-hex after ends the escape). Use this when adding a character to a CSS `content:` rule or a class name selector." },
        { q: "Why are some characters not searchable by keyword?", a: "Each entry has its full name and an optional `keywords` array. We tagged the most-commonly-searched ones (e.g. 'mac' finds ⌘ via the 'command' keyword); contributions welcome on GitHub for more." },
        { q: "Does pasting a character into search work?", a: "Yes — paste any character from the catalogue into the search box and you'll see its details immediately." },
        { q: "Can I export the catalogue?", a: "Not in this version — the catalogue is a static module. Anything you copy via the focused panel works as a building block." },
        { q: "Is anything uploaded?", a: "No. The catalogue is bundled with the page and search runs entirely in your browser." },
      ],
    },
  },
  {
  ...
  },
  {
    ...
    seo: {
      ...
      how: [
        "Add fields to the schema — pick a name and a type for each.",
        "Set min/max for numbers, sentence count for lorem, or a literal value if needed.",
        "Reorder fields with the up/down buttons; remove with the trash icon.",
        "Pick the record count (1-10 000) and click Regenerate. Copy or download the JSON.",
      ],
      benefits: [
      ...
      faqs: [
        ...
        { q: "How is the UUID generated?", a: "We generate 16 random bytes, then force the version 4 bits (top 4 of byte 6 = `0100`) and variant bits (top 2 of byte 8 = `10`) per RFC 4122. The result is a valid v4 UUID indistinguishable from one generated by any standard library." },
        { q: "Can I generate millions of records?", a: "Up to 10,000 per click. For more, click Regenerate again — there's no rate limit because everything's local. The cap exists to keep the browser tab from running out of memory." },
        { q: "What's the 'literal' field type for?", a: "When you want a constant value across all records — useful for tenant IDs, environment markers, version numbers etc. Set the literal once and it appears in every generated record." },
        { q: "How do I get nested objects?", a: "Not in this version — the generator produces flat objects with primitive values. For nested shapes, generate two arrays and combine them in your application." },
        { q: "Does the output round-trip through JSON.parse?", a: "Yes — every value is JSON-safe (string, number, boolean, ISO date string). Pasting the output into JSON.parse() works without modification." },
        { q: "Will the same schema produce different data each time?", a: "Yes — each regeneration uses fresh random bytes. Click Regenerate to mint a new batch." },
        { q: "Can I seed the random for reproducible output?", a: "Not in this version. Crypto.getRandomValues is unseedable by design. For a reproducible fixture, generate once and save the output." },
        { q: "Are emails / phones routable?", a: "No. Emails use placeholder domains (example.com, test.dev, fake.io). Phones are constructed from common country codes plus random digits and aren't real numbers." },
  ...
  // ─── CONVERTERS (batch 2) ────────────────────────────────────────────────
  {
    ...
    seo: {
      ...
      faqs: [
        ...
        { q: "Will it handle Windows line endings (CRLF)?", a: "Yes — both CRLF and LF are recognised as row terminators. The parser doesn't care which you use." },
        { q: "What's the difference between 'array of objects' and 'array of arrays'?", a: "Array of objects: `[{ id: 1, name: 'Ada' }, … ]` — each row is a JSON object keyed by the header. Array of arrays: `[[1, 'Ada'], … ]` — each row is a JSON array, preserving column order without naming fields. Pick the first for API responses, the second when column order matters more than field names." },
        { q: "How does delimiter auto-detection work?", a: "We count occurrences of comma, semicolon, tab and pipe in the first 10 lines and pick whichever appears most. Override manually if your data is unusual." },
        { q: "What types are detected?", a: "Integers (`-?\\d+`), decimals (`-?\\d+\\.\\d+`, optional scientific), booleans (`true` / `false` / `TRUE` / `FALSE`) and `null` / `NULL`. Everything else stays a string. Turn the toggle off if you want strict strings." },
        { q: "Why is my row showing extra fields?", a: "Probably an unescaped delimiter inside one of your fields. The 'inconsistent field counts' warning is a hint — find the row, wrap the offending field in double quotes." },
        { q: "What if the header has duplicate names?", a: "We keep them; the JSON object will have the second occurrence overwrite the first (JavaScript object semantics). Use 'array of arrays' if duplicates matter." },
        { q: "Can I convert in the other direction?", a: "Yes — see JSON to CSV Converter (the companion tool)." },
        { q: "How large a CSV can it parse?", a: "Comfortable up to ~10 MB / 100 000 rows in modern browsers. The parser is O(n); the bottleneck is browser memory." },
        { q: "Is anything uploaded?", a: "No. Parsing and JSON conversion run entirely in your browser." },
      ],
    },
  },
  {
    ...
    seo: {
      ...
      faqs: [
        ...
        { q: "What about nested arrays?", a: "Three modes: 'Stringify as JSON' (lossless — `[\"a\",\"b\"]`), 'Join with `|`' (readable — `a | b`), 'Take first item' (when only the first element matters)." },
        { q: "Why use CRLF newlines?", a: "Excel imports CSV cleanly with CRLF. LF works for most other tools but Excel sometimes interprets it as one giant row. Default is LF — switch to CRLF if you're heading to Excel." },
        { q: "What gets quoted?", a: "Per RFC 4180: fields containing the delimiter, a double quote, or a newline. With 'Always quote' on, every field is wrapped — useful for downstream tools that always expect quotes." },
        { q: "What if rows have different fields?", a: "We take the union of all keys across rows and use that as the header. Rows missing a field show an empty cell for that column. No crash." },
        { q: "What happens to null and undefined?", a: "Both become empty cells. Booleans become `true` / `false`. Strings, numbers and JSON-stringified objects/arrays are passed through." },
        { q: "Does it preserve column order?", a: "Mostly — we follow object key order (JavaScript insertion order). The first object's keys appear first; new keys from later objects append in the order they're discovered." },
        { q: "Can I pick specific columns?", a: "Not via a UI selector — but if you preprocess your JSON to keep only the keys you want (or use `Object.fromEntries` to rename), the output reflects that exactly." },
        { q: "Will it round-trip through CSV to JSON?", a: "Yes for flat data. Nested objects/arrays lose type info when round-tripped — the CSV cell becomes a string. Use the JSON-stringify nested-array mode and the typed-values option in CSV to JSON for the cleanest round-trip." },
        { q: "Is anything uploaded?", a: "No. Conversion runs entirely in your browser." },
      ],
    },
  },
  {
  ...
  },
  {
    ...
    seo: {
      ...
      faqs: [
        ...
        { q: "Will it preserve tables?", a: "Yes — table headers, rows and per-column alignment (from `align=` or `style=\"text-align:…\"`) all carry through to GFM tables." },
        { q: "What about nested lists?", a: "Yes. Lists of any nesting depth are converted; the indent is `2 × depth` spaces per the GFM convention." },
        { q: "Does it handle the task-list extension?", a: "Yes. An `<input type=\"checkbox\" checked>` inside a list item becomes `- [x]` and unchecked becomes `- [ ]`." },
        { q: "What about code blocks?", a: "`<pre><code class=\"language-ts\">…</code></pre>` becomes a fenced code block with the `ts` language tag. Inline `<code>` uses adaptive backtick counting so embedded backticks don't break the output." },
        { q: "Will it strip my CSS?", a: "Yes — `<style>` blocks and inline `style` attributes are dropped (we don't try to convert CSS to anything Markdown can render)." },
        { q: "What if my input has unknown tags?", a: "By default we strip them (passing through their children). Toggle 'keep unknown HTML tags' on if you want them preserved as raw HTML in the Markdown output." },
        { q: "Does it work on broken HTML?", a: "DOMParser is lenient — it produces a valid tree from almost anything. Unclosed tags, mismatched nesting, missing quotes all get tolerated the same way browsers tolerate them." },
        { q: "Is my HTML uploaded?", a: "No. Parsing and emission run entirely in your browser." },
        { q: "Can I convert in the other direction?", a: "Yes — see the Markdown to HTML tool." },
      ],
    },
  },
  {
  ...
  },
  {
    ...
    seo: {
      ...
      how: [
        "Type a number — with or without a `0x` / `0o` / `0b` prefix.",
        "Pick a source base hint (or leave on auto-detect).",
        "Read all four base representations side-by-side.",
        "Toggle UPPERCASE hex, digit grouping, and step-by-step conversion display.",
      ],
      benefits: [
      ...
      faqs: [
        ...
        { q: "What's the difference between `0x` and `0X`?", a: "None — we accept both. Same for `0o`/`0O`, `0b`/`0B`." },
        { q: "What's the largest number it handles?", a: "BigInt has no fixed maximum — limited only by browser memory. Numbers with thousands of digits work, just slowly. Display gets unwieldy past a few hundred digits." },
        { q: "What does 'fits u32' mean?", a: "Whether the value fits in a 32-bit unsigned integer (0 to 2^32 - 1 = 4,294,967,295). Useful when feeding values into systems that expect 32-bit ints." },
        { q: "And 'fits i64'?", a: "Whether the value fits in a 64-bit signed integer (-2^63 to 2^63 - 1). Most modern languages use i64 as their default integer type." },
        { q: "How do I group binary digits?", a: "Toggle 'Group digits' — binary groups into 4-bit nibbles (a hex digit each), octal into 3-bit groups, decimal into 3-digit groups." },
        { q: "Why uppercase hex by default?", a: "It's the convention in most programming languages, RFC documents and crypto libraries. Toggle off for lowercase output." },
        { q: "Show me the conversion steps?", a: "Enable 'Show conversion steps' — we print a table of the divide-by-16 algorithm: at each step we divide the current value by 16 and record the remainder. Read the remainders bottom-up to get the hex digits." },
        { q: "Does it handle floating-point numbers?", a: "Not in this version — BigInt is integers only. For float-to-hex (IEEE 754 representation), you'd need a specialised tool." },
        { q: "Is anything uploaded?", a: "No. Conversion is a pure browser computation." },
      ],
    },
  },
  {
    ...
    seo: {
      ...
      faqs: [
        ...
        { q: "Is 'IIII' a valid Roman numeral?", a: "On clock faces, yes — many clocks show IIII instead of IV for aesthetic balance. Strictly, no: the canonical form for 4 is IV. Our parser rejects IIII." },
        { q: "What about 'IC' for 99?", a: "Invalid. 99 is XCIX (90 + 9). The only valid subtractive pairs are IV, IX, XL, XC, CD, CM — each pair has the smaller-by-factor-of-5 or 10 immediately before a larger." },
        { q: "How do I type the overline?", a: "When converting Roman → Decimal, you can paste a vinculum'd Roman from elsewhere (the U+0305 combining macron is preserved). Our parser also accepts the version where each macron immediately follows a letter." },
        { q: "How do I write the year 2026?", a: "MMXXVI (1000 + 1000 + 10 + 10 + 5 + 1)." },
        { q: "And the famous 1947?", a: "MCMXLVII (1000 + (1000-100) + (50-10) + 5 + 1 + 1)." },
        { q: "How are years like 1990 written?", a: "MCMXC. Not MXM (which isn't valid). The subtractive rule applies one place at a time — 90 = XC, 900 = CM." },
        { q: "Does it support Unicode Roman numeral characters (Ⅰ, Ⅱ, Ⅲ)?", a: "Not in this version. The Unicode Roman numeral block (U+2160–U+217F) is supported by very few fonts cleanly. We output ASCII Roman letters which render everywhere." },
        { q: "What about zero?", a: "Romans had no symbol for zero. The minimum input is 1." },
        { q: "Is anything uploaded?", a: "No. Conversion is a pure browser computation." },
      ],
    },
  },
  {
    ...
    seo: {
      ...
      how: [
        "Pick a direction: Text → Morse or Morse → Text.",
        "Set the speed (WPM) and tone frequency (Hz).",
        "Type or paste the text / Morse. The output updates live.",
        "Press Play to hear the Morse synthesised by the Web Audio API.",
      ],
      benefits: [
      ...
      relatedSlugs: ["nato-alphabet-converter", "braille-translator", "roman-numeral-converter", "binary-to-decimal"],
      faqs: [
        { q: "What's the standard speed?", a: "Beginners learn at 5 WPM, intermediate ham radio operators work at 13-20 WPM, advanced operators handle 25+ WPM. Real-world CW (continuous wave) traffic on the bands hovers around 15-25 WPM. The default here is 15." },
        { q: "How is WPM calculated?", a: "The PARIS-word reference: a 50-unit word at 1 WPM = 1.2 s per unit. So at 15 WPM, one dot = 80 ms, one dash = 240 ms, intra-letter gap = 80 ms, inter-letter gap = 240 ms, inter-word gap = 560 ms." },
        { q: "What's the difference between American and International Morse?", a: "International (ITU) Morse is the standard everywhere except in some niche US contexts. We use ITU exclusively — no obscure American Morse oddities." },
        { q: "Why 600 Hz default tone?", a: "Most ham radio rigs offer 500-800 Hz as the CW sidetone sweet spot. 600 Hz is the middle and what many operators describe as 'pleasant to copy for hours'." },
        { q: "Does it support prosigns like SK / BT / AR?", a: "Not as named prosigns — but you can encode them by chaining letters without spacing on the input side, e.g. type 'SK' for VA (end of work)." },
        { q: "What about SOS?", a: "It's a prosign — three dots, three dashes, three dots, traditionally with no inter-letter gap (`...---...`). For voice-pronounceable use, just type `SOS`; for the prosign version, type `...---...` directly into the Morse input." },
        { q: "How accurate is the audio timing?", a: "We schedule oscillator start / stop on the Web Audio timeline which is sample-accurate. So 15 WPM is exactly 15 WPM down to the millisecond." },
        { q: "What about Farnsworth timing?", a: "Not implemented — Farnsworth sends individual letters at a faster speed with longer inter-letter and inter-word gaps. The current playback uses standard PARIS timing." },
        { q: "Can I download the audio?", a: "Not in this version — the audio is generated live. To record, use any system audio capture tool while pressing Play." },
        { q: "Does playback work on mobile?", a: "Yes — Web Audio is supported on iOS Safari and Chrome / Firefox on Android. The first Play may require a user tap (browser autoplay policies)." },
        { q: "Is anything uploaded?", a: "No. Translation and audio synthesis run entirely in your browser." },
      ],
    },
  },
  {
    ...
    seo: {
      ...
      relatedSlugs: ["morse-code-translator", "nato-alphabet-converter", "fancy-text-generator", "unicode-character-finder"],
      faqs: [
        { q: "What's Grade 1 vs Grade 2 Braille?", a: "Grade 1 is letter-by-letter (uncontracted) Braille — each English letter maps to exactly one Braille cell. Grade 2 includes contractions (single cells that stand for common words like 'and', 'the', 'with') for faster reading. We implement Grade 1 only." },
        { q: "Is this real Braille that a screen reader will read?", a: "It's a visual approximation in Unicode. Real refreshable Braille displays read from a digital text source — what looks like Braille on screen here can be embossed using a Braille printer, but most screen readers will read the underlying Unicode code points (which they recognise as Braille cells in their output)." },
        { q: "Does it handle uppercase letters?", a: "Yes — when 'use capital indicator' is on, every uppercase letter is preceded by ⠠. Turn it off if your downstream tool expects lowercase letters only." },
        { q: "How are numbers written?", a: "Digits 1-9 share cell patterns with A-I; 0 uses J's pattern. To disambiguate from letters, the number indicator ⠼ prefixes any digit run. The first non-digit pattern ends the digit run." },
        { q: "What about contractions like 'and' or 'the'?", a: "Those are Grade 2 contractions and are NOT implemented in this tool. The full English Grade 2 system has 180+ contractions and is best done by a dedicated Braille library." },
        { q: "What's the Unicode block?", a: "U+2800 through U+28FF — Braille Patterns. Each codepoint is a unique 8-dot cell. We use only the 64 patterns covered by the standard 6-dot Braille system (the 7th and 8th dots are for Computer Braille extensions)." },
        { q: "Does it work on Spanish, French, German Braille?", a: "No — only English Braille is implemented. The basic 26-letter alphabet works, but accented characters used in other languages have different Braille mappings that aren't included." },
        { q: "Will the output paste correctly?", a: "Yes — Unicode Braille Patterns are well-supported in modern fonts. Some older fonts may show the cells as boxes, but copy-paste preserves the actual code points." },
        { q: "What if my input contains emoji?", a: "Emoji pass through unchanged — they're not in the English Braille alphabet so we leave them alone." },
        { q: "How do I learn the alphabet?", a: "The reference grid at the bottom shows every letter, digit and indicator with its Braille glyph. Each cell follows the standard 1-2-3 / 4-5-6 dot layout." },
        { q: "Is anything uploaded?", a: "No. Translation is a pure browser computation." },
      ],
    },
  },
  {
    ...
    seo: {
      ...
      how: [
        "Pick a direction: Text → NATO or NATO → Text.",
        "Type or paste your input. The output and letter-by-letter table update live.",
        "Choose a joiner (space, dash, dot, comma) and casing (sentence vs UPPER).",
        "Copy or download the result. Use the letter-by-letter table when dictating.",
      ],
      benefits: [
      ...
      relatedSlugs: ["morse-code-translator", "braille-translator", "roman-numeral-converter", "fancy-text-generator"],
      faqs: [
        { q: "Why does the NATO alphabet exist?", a: "To avoid letter confusion over noisy voice comms. B and D, M and N, F and S all sound similar over a poor radio link. Spelling each letter as a distinct word (Bravo / Delta) makes the meaning unambiguous." },
        { q: "Why 'Wun, Too, Tree, Fower, Fife, Niner'?", a: "Distinct phonetic forms for digits — the standard 'one', 'two', etc. can sound similar to other words. 'Niner' specifically distinguishes from German 'nein' (no) — crucial in NATO's multilingual environment." },
        { q: "Is it 'Juliett' or 'Juliet'?", a: "The official NATO spelling is 'Juliett' with two t's — to avoid French pronunciation as 'zhoo-lee-ay'. Our reverse parser accepts both Juliett, Juliet and Juliette." },
        { q: "What about 'Whiskey' vs 'Whisky'?", a: "NATO uses 'Whiskey'. We accept both spellings on decode." },
        { q: "Does this match the British / NATO / aviation versions?", a: "NATO, ICAO, FAA, IMO and Australia / Canada / UK / US military all use the same 1956 alphabet. So yes — it's universal across English-speaking aviation and military comms." },
        { q: "What about non-English alphabets?", a: "The 1956 alphabet is English-centric. Other languages have their own (Polish has Anna Bolesław Celina; Italian uses city names). We don't include them in this tool." },
        { q: "Can I dictate a phone number with this?", a: "Yes — turn off 'spell punctuation' and the joiner to space, paste your phone number, and you get the digit words. E.g. '415-555-1234' → 'Fower Wun Fife Fife Fife Fife Wun Too Tree Fower' (with the dashes dropped if 'spell punctuation' is off)." },
        { q: "Why does my decode show '?xyz?' for some words?", a: "We surface unknown tokens explicitly so you can spot typos. If 'Charlee' (misspelled) appears in your input, it shows as '?charlee?' in the output rather than being silently dropped." },
        { q: "What's the joiner for?", a: "How the NATO words are joined. Space (`Alpha Bravo Charlie`) is the default. Dash (`Alpha - Bravo - Charlie`) is common in printed docs. Hyphen (`Alpha-Bravo-Charlie`) is occasionally seen. Middle dot (`Alpha · Bravo · Charlie`) is just stylistic." },
        { q: "What happens to spaces in my input?", a: "We mark them explicitly with `(space)` so the letter-by-letter dictation is unambiguous. Drop 'punctuation' to skip them." },
        { q: "Is anything uploaded?", a: "No. Conversion is a pure browser computation." },
      ],
    },
  },
