...
import { DECISION_PRESETS } from "@/lib/tools/wheel/presets";

const SETTINGS_KEY = "toollyz:decision-settings";
...
export default function DecisionMakerWheel() {
  const reduceMotion = useReducedMotion();
  const [options, setOptions] = React.useState<string[]>(
    DECISION_PRESETS[0].options,
  );
  const [draft, setDraft] = React.useState("");
  const [themeId, setThemeId] = React.useState("birthday");
  const [eliminate, setEliminate] = React.useState(false);
  const [soundOn, setSoundOn] = React.useState(true);
  const [activePreset, setActivePreset] = React.useState<string | null>("lunch");
  ...
  React.useEffect(() => {
    try {
      ...
      if (s) {
        ...
        if (Array.isArray(p.options) && p.options.length) setOptions(p.options);
        if (p.themeId) setThemeId(p.themeId);
        if (typeof p.soundOn === "boolean") setSoundOn(p.soundOn);
        if (p.activePreset !== undefined) setActivePreset(p.activePreset);
      }
    } catch {
      /* noop */
    }
  ...
  React.useEffect(() => {
    try {
      localStorage.setItem(
        SETTINGS_KEY,
        JSON.stringify({ options, themeId, soundOn, activePreset }),
      );
    } catch {
      /* noop */
    }
  }, [options, themeId, soundOn, activePreset]);
  ...
  function scheduleTicks(duration: number) {
    ...
    while (t < duration * 1000) {
      tickTimers.current.push(window.setTimeout(() => playTick(), t));
      t += interval;
      interval *= 1.12;
    }
  }
  ...
  const spin = React.useCallback(() => {
    ...
    const idx = fairIndex(options.length);
    setSpinning(true);
    setWinner(null);
    const duration = reduceMotion ? 0.6 : 4 + Math.random() * 2;
    const target = computeTargetRotation(rotation, idx, options.length, reduceMotion ? 2 : 6);
    setRotation(target);
    scheduleTicks(duration);

    window.setTimeout(
      () => {
        const win = options[idx];
        setWinner(win);
        setSpinning(false);
        setShowModal(true);
        setConfetti(true);
        ...
        setHistory((prev) =>
          [{ id: crypto.randomUUID(), name: win, at: Date.now() }, ...prev].slice(0, 50),
        );
      },
      duration * 1000 + 100,
    );
  }, [spinning, options, rotation, reduceMotion, soundOn]);
  ...
  function addDraft() {
    ...
    setOptions((prev) => Array.from(new Set([...prev, ...items])));
    setDraft("");
  ...
  function dedupe() {
    setOptions((prev) => {
      ...
      else toast.info("No duplicates");
      return unique;
    });
  }

  function acceptWinner() {
    if (eliminate && winner) setOptions((prev) => prev.filter((e) => e !== winner));
    setShowModal(false);
  }
  ...
  function exportPng() {
    const svg = document.getElementById("decision-wheel-svg");
    if (!svg) return;
    ...
    img.onload = () => {
      ...
      canvas.toBlob((blob) => {
        ...
        a.href = url;
        a.download = `toollyz-decision-${Date.now()}.png`;
  ...
  function exportJson() {
    const blob = new Blob(
      [JSON.stringify({ options, theme: themeId, history }, null, 2)],
      { type: "application/json;charset=utf-8" },
    );
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url;
    a.download = `toollyz-decision-${Date.now()}.json`;
    document.body.appendChild(a);
    a.click();
    document.body.removeChild(a);
    URL.revokeObjectURL(url);
  ...
  return (
    <div className="space-y-6">
      {/* ─── Preset templates ─────────────────────────────────────── */}
      <section aria-label="Quick decision presets" className="space-y-3">
      ...
      <div className="grid gap-6 lg:grid-cols-[1fr_1.1fr]">
        ...
        <div className="space-y-5">
          <div className="space-y-2">
            <Label htmlFor="decision-draft">Your options</Label>
            ...
            <Textarea
              value={options.join("\n")}
              onChange={(e) => {
                setOptions(parseEntries(e.target.value));
                setActivePreset(null);
              }}
              placeholder={"Or paste options — one per line…"}
              rows={5}
            ...
            <div className="flex flex-wrap items-center justify-between gap-2 text-xs text-muted-foreground">
              ...
              <div className="flex gap-1.5">
                <Button type="button" variant="ghost" size="sm" onClick={shuffleOptions}>
          ...
          <div className="flex flex-wrap gap-2">
            <Button type="button" variant="outline" size="sm" onClick={exportPng} disabled={options.length < 2}>
              <Download className="size-3.5" />
              PNG
            </Button>
            <Button type="button" variant="outline" size="sm" onClick={exportJson} disabled={!options.length}>
              <Download className="size-3.5" />
              JSON
            </Button>
          </div>
        ...
        <div className="flex flex-col items-center gap-5">
          ...
          <div className="flex flex-col items-center gap-2">
            <Button
              ...
              className="min-w-44"
            >
              <Zap className={cn("size-4", spinning && "animate-pulse")} />
      ...
      {/* History */}
      {history.length > 0 && (
        <section aria-label="Decision history" className="space-y-3">
      ...
      <AnimatePresence>
        {showModal && winner && (
          <WinnerModal
            ...
            onSpinAgain={() => {
              setShowModal(false);
              if (eliminate) setOptions((prev) => prev.filter((e) => e !== winner));
...
function DecisionWheel({
  ...
  return (
    <div className="relative aspect-square w-full max-w-[360px]">
      ...
      <motion.svg
        id="decision-wheel-svg"
        viewBox={`0 0 ${size} ${size}`}
        className="size-full cursor-pointer"
        onClick={() => !spinning && onSpin()}
        animate={{ rotate: rotation }}
        transition={{ duration, ease: spinning ? [0.16, 1, 0.3, 1] : "linear" }}
    ...
    >
      ...
      >
        ...
        <motion.div
          ...
          className="mx-auto grid size-16 place-items-center rounded-2xl bg-gradient-to-br from-indigo-500 to-violet-600 text-white shadow-lg"
        >
