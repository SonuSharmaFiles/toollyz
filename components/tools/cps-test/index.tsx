...
const STORAGE = { key: "toollyz:cps-history" };
...
export default function CpsTest() {
  ...
  function finishRun(seconds: number) {
    ...
    toast.success(`${count} clicks in ${duration}s · ${final} CPS`);
  }
    ...
    <div className="space-y-6">
      ...
      >
        ...
        <div className="relative grid grid-cols-2 gap-4 sm:grid-cols-4">
          <Stat label="Clicks" value={count} reduceMotion={!!reduceMotion} accent="text-emerald-300" />
          <Stat label="CPS" value={liveCps} suffix="" reduceMotion={!!reduceMotion} accent="text-indigo-300" decimals={2} />
      ...
      <section className="space-y-3 rounded-2xl border border-border/70 bg-card p-4">
        ...
        <p className="text-[11px] text-muted-foreground">
          {phase === "idle" && "Click the big button to start — the timer begins on the first click."}
          {phase === "running" && "Keep clicking until the timer hits zero."}
      ...
      <section className="rounded-2xl border border-border/60 bg-muted/30 p-4">
        ...
        <ul className="grid gap-1.5 text-xs text-muted-foreground sm:grid-cols-2 list-none">
          <li className="flex items-start gap-1.5"><MousePointerClick className="mt-0.5 size-3.5 shrink-0 text-primary" />Each click on the button counts. The timer starts on the first click and stops automatically.</li>
          <li className="flex items-start gap-1.5"><Timer className="mt-0.5 size-3.5 shrink-0 text-primary" />Five durations (1, 5, 10, 30, 60 seconds). Best score per duration surfaces in the hero.</li>
          <li className="flex items-start gap-1.5"><Sparkles className="mt-0.5 size-3.5 shrink-0 text-primary" />Holds and rapid double-clicks count as separate events as long as the browser dispatches a click for each.</li>
          <li className="flex items-start gap-1.5"><Lock className="mt-0.5 size-3.5 shrink-0 text-primary" />Toollyz has no backend — every click stays in your browser. The 30 most recent runs save to localStorage.</li>
        </ul>
      </section>
